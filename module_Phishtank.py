...
class Phishtank(Module):
    
    def __init__(self):
        path = './data/verified_online.csv'
        ...
        if result != "uploaded":
            for index, row in data.iterrows():
                rds.set(name + ":" + row["phish_detail_url"], "detected")
                rds.set(name + ":" + row["url"], "detected")
